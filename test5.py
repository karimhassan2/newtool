...
def read_url_file(path):
    try:
        ...
        raise SystemExit(f'--file-urls: cannot read {path}: {e}')
    ...
    for lineno, raw in enumerate(text.splitlines(), 1):
        ...
        if urlparse(line).scheme not in ('http', 'https'):
            log(f'[SKIP] {path}:{lineno}: not an http(s) URL: {line}', always=True)
            continue
        urls.append(line)
...
def args(argv=None):
    ...
    p.add_argument(
        ...
        default=[],
        help=(
            'Authorized http(s) URL, local file/directory, or - for stdin. '
            'Optional if --file-urls is given.'
        )
    )
    ...
    p.add_argument(
        ...
        metavar='FILE',
        help=(
            'File containing one authorized http(s) URL per line (# comments '
            'and blank lines ignored) to add as scan seeds - e.g. a js_urls.txt '
            'produced by a separate subdomain/URL-discovery step. Repeatable.'
        )
    )
...
def main(argv=None):
    ...
    if not a.targets and not a.file_urls:
        raise SystemExit('no targets: pass at least one target or --file-urls FILE')
    ...
    for fp in a.file_urls:
        remote+=read_url_file(fp)

    remote=list(dict.fromkeys(remote))  # de-dup, keep order
